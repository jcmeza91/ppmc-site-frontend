...
define(function () {
    return ['$scope', 'appVersion', 'servicePHP', '$filter','$http', 'globalVariable',function ($scope, appVersion, servicePHP, $filter,$http,globalVariable) {
        ...
        servicePHP.requestAPI(parameters,"crew","GET","Obteniendo registros").then(function(response){
        ...
        $scope.add = function(){
            window.location.href = '#/tripulacion';  	
        }
        
        $scope.edit = function(item){
            globalVariable.setIdItem(item.crewId);
            window.location.href = '#/editarTripulacion';  	
        }
