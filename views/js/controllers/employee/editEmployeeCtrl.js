...
define(function () {
    return ['$scope', 'servicePHP', 'globalVariable', '$http', function ($scope, servicePHP, globalVariable, $http) {
        ...
        servicePHP.requestAPI(parameters,"employee/"+globalVariable.getIdItem(),"GET","Obteniendo Datos").then(function(response){
    	...
    	$scope.update=function(parameters){
            servicePHP.requestAPI(parameters,"employee/"+globalVariable.getIdItem(),"PUT","Actualizando Registro").then(function(response){
                if (response.statusText=="OK"){
