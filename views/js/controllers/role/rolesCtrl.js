...
define(function () {
    return ['$scope', 'appVersion', 'servicePHP', '$filter','$http', 'globalVariable',function ($scope, appVersion, servicePHP, $filter,$http,globalVariable) {
        ...
        servicePHP.requestAPI(parameters,"role","GET","Obteniendo registros").then(function(response){
        ...
        $scope.delete = function(item,index){
            ...
            servicePHP.requestAPI(parameters,"role/"+item.roleId,"DELETE","Removiendo registro").then(function(response){
                if (response.statusText=="OK"){
        ...
        $scope.edit = function(item){
            globalVariable.setIdItem(item.roleId);
