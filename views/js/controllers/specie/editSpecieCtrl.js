...
define(function () {
    return ['$scope', 'servicePHP', 'globalVariable', '$http', function ($scope, servicePHP, globalVariable, $http) {
        ...
        servicePHP.requestAPI(parameters,"specie/"+globalVariable.getIdItem(),"GET","Obteniendo Datos").then(function(response){
    	...
    	$scope.update=function(parameters){
            servicePHP.requestAPI(parameters,"specie/"+globalVariable.getIdItem(),"PUT","Actualizando Registro").then(function(response){
                if (response.statusText=="OK"){
